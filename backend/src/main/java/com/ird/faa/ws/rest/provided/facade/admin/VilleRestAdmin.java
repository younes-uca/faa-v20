...
private VilleConverter villeConverter;
    ...
    public List<VilleVo> findByCriteria(@RequestBody VilleVo villeVo){
        ...
        public List<Ville> findByPaysCode(@PathVariable String code){
        return villeService.findByPaysCode(code);
        }

        @ApiOperation("delete by pays code")
        @DeleteMapping("/pays/code/{code}")
        public int deleteByPaysCode(@PathVariable String code){
        return villeService.deleteByPaysCode(code);
        }
        ...
        return villeService.deleteByPaysId(id);
        }



            @PutMapping("/archiver/")
            public VilleVo archiver(@RequestBody VilleVo villeVo){
                Ville ville = villeService.archiver(villeConverter.toItem(villeVo));
                return villeConverter.toVo(ville);
                }

            @PutMapping("/desarchiver/")
            public VilleVo desarchiver(@RequestBody VilleVo villeVo){
                Ville ville = villeService.desarchiver(villeConverter.toItem(villeVo));
                return villeConverter.toVo(ville);}
            }
