...
private StructureIrdConverter structureIrdConverter;
    ...
    public List<StructureIrdVo> findByCriteria(@RequestBody StructureIrdVo structureIrdVo){
        return structureIrdConverter.toVo(structureIrdService.findByCriteria(structureIrdVo));
            ...
            @PutMapping("/archiver/")
            public StructureIrdVo archiver(@RequestBody StructureIrdVo structureIrdVo){
                StructureIrd structureIrd = structureIrdService.archiver(structureIrdConverter.toItem(structureIrdVo));
                return structureIrdConverter.toVo(structureIrd);
                }

            @PutMapping("/desarchiver/")
            public StructureIrdVo desarchiver(@RequestBody StructureIrdVo structureIrdVo){
                StructureIrd structureIrd = structureIrdService.desarchiver(structureIrdConverter.toItem(structureIrdVo));
                return structureIrdConverter.toVo(structureIrd);}
            }
