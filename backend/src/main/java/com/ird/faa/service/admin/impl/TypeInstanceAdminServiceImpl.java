...
return findById(id);
     ...
     @Override
    public TypeInstance archiver(TypeInstance typeInstance) {
        if (typeInstance.getArchive() == null) {
        typeInstance.setArchive(false);
        }
        typeInstance.setArchive(true);
        typeInstance.setDateArchivage(new Date());
        typeInstanceDao.save(typeInstance);
        return typeInstance;

    }

    @Override
    public TypeInstance desarchiver(TypeInstance typeInstance) {
    if (typeInstance.getArchive() == null) {
    typeInstance.setArchive(false);
    }
    typeInstance.setArchive(false);
    typeInstance.setDateArchivage(null);
    typeInstanceDao.save(typeInstance);
    return typeInstance;
    }
