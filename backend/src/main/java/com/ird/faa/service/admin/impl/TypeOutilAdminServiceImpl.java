...
return findById(id);
     ...
     @Override
    public TypeOutil archiver(TypeOutil typeOutil) {
        if (typeOutil.getArchive() == null) {
        typeOutil.setArchive(false);
        }
        typeOutil.setArchive(true);
        typeOutil.setDateArchivage(new Date());
        typeOutilDao.save(typeOutil);
        return typeOutil;

    }

    @Override
    public TypeOutil desarchiver(TypeOutil typeOutil) {
    if (typeOutil.getArchive() == null) {
    typeOutil.setArchive(false);
    }
    typeOutil.setArchive(false);
    typeOutil.setDateArchivage(null);
    typeOutilDao.save(typeOutil);
    return typeOutil;
    }
