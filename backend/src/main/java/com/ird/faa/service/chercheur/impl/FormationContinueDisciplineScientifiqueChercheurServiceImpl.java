...
import com.ird.faa.dao.FormationContinueDisciplineScientifiqueDao;
...
@Service
public class FormationContinueDisciplineScientifiqueChercheurServiceImpl extends AbstractServiceImpl<FormationContinueDisciplineScientifique> implements FormationContinueDisciplineScientifiqueChercheurService {
...
@Override
public FormationContinueDisciplineScientifique update(FormationContinueDisciplineScientifique formationContinueDisciplineScientifique){
FormationContinueDisciplineScientifique foundedFormationContinueDisciplineScientifique = findById(formationContinueDisciplineScientifique.getId());
if(foundedFormationContinueDisciplineScientifique==null) return null;
else{
...
@Override
public FormationContinueDisciplineScientifique save (FormationContinueDisciplineScientifique formationContinueDisciplineScientifique){
...
@Override
public List<FormationContinueDisciplineScientifique> save(List<FormationContinueDisciplineScientifique> formationContinueDisciplineScientifiques){
...
public int delete(FormationContinueDisciplineScientifique formationContinueDisciplineScientifique){
    if(formationContinueDisciplineScientifique.getId()==null) return -1;
    FormationContinueDisciplineScientifique foundedFormationContinueDisciplineScientifique = findById(formationContinueDisciplineScientifique.getId());
    if(foundedFormationContinueDisciplineScientifique==null) return -1;
formationContinueDisciplineScientifiqueDao.delete(foundedFormationContinueDisciplineScientifique);
return 1;
}


public List<FormationContinueDisciplineScientifique> findByCriteria(FormationContinueDisciplineScientifiqueVo formationContinueDisciplineScientifiqueVo){
...
String query = "SELECT o FROM FormationContinueDisciplineScientifique o where 1=1 ";
    ...
    if(formationContinueDisciplineScientifiqueVo.getFormationContinueVo()!=null){
        query += SearchUtil.addConstraint( "o", "formationContinue.id","=",formationContinueDisciplineScientifiqueVo.getFormationContinueVo().getId());
    }

    if(formationContinueDisciplineScientifiqueVo.getDisciplineScientifiqueVo()!=null){
        query += SearchUtil.addConstraint( "o", "disciplineScientifique.id","=",formationContinueDisciplineScientifiqueVo.getDisciplineScientifiqueVo().getId());
            query += SearchUtil.addConstraint( "o", "disciplineScientifique.code","LIKE",formationContinueDisciplineScientifiqueVo.getDisciplineScientifiqueVo().getCode());
    }

return entityManager.createQuery(query).getResultList();
}

    private void findFormationContinue(FormationContinueDisciplineScientifique formationContinueDisciplineScientifique){
        FormationContinue loadedFormationContinue = null;
        if(formationContinueDisciplineScientifique.getFormationContinue() != null && formationContinueDisciplineScientifique.getFormationContinue().getId() !=null)
        loadedFormationContinue =formationContinueService.findById(formationContinueDisciplineScientifique.getFormationContinue().getId());
    ...
    private void findDisciplineScientifique(FormationContinueDisciplineScientifique formationContinueDisciplineScientifique){
        DisciplineScientifique loadedDisciplineScientifique =disciplineScientifiqueService.findByIdOrCode(formationContinueDisciplineScientifique.getDisciplineScientifique());
