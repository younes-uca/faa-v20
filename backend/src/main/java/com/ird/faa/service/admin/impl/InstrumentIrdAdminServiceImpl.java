...
return findById(id);
     ...
     @Override
    public InstrumentIrd archiver(InstrumentIrd instrumentIrd) {
        if (instrumentIrd.getArchive() == null) {
        instrumentIrd.setArchive(false);
        }
        instrumentIrd.setArchive(true);
        instrumentIrd.setDateArchivage(new Date());
        instrumentIrdDao.save(instrumentIrd);
        return instrumentIrd;

    }

    @Override
    public InstrumentIrd desarchiver(InstrumentIrd instrumentIrd) {
    if (instrumentIrd.getArchive() == null) {
    instrumentIrd.setArchive(false);
    }
    instrumentIrd.setArchive(false);
    instrumentIrd.setDateArchivage(null);
    instrumentIrdDao.save(instrumentIrd);
    return instrumentIrd;
    }
