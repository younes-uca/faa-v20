...
return findById(id);
     ...
     @Override
    public StructureIrd archiver(StructureIrd structureIrd) {
        if (structureIrd.getArchive() == null) {
        structureIrd.setArchive(false);
        }
        structureIrd.setArchive(true);
        structureIrd.setDateArchivage(new Date());
        structureIrdDao.save(structureIrd);
        return structureIrd;

    }

    @Override
    public StructureIrd desarchiver(StructureIrd structureIrd) {
    if (structureIrd.getArchive() == null) {
    structureIrd.setArchive(false);
    }
    structureIrd.setArchive(false);
    structureIrd.setDateArchivage(null);
    structureIrdDao.save(structureIrd);
    return structureIrd;
    }
