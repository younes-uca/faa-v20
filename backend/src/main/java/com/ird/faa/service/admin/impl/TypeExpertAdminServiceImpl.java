...
return findById(id);
     ...
     @Override
    public TypeExpert archiver(TypeExpert typeExpert) {
        if (typeExpert.getArchive() == null) {
        typeExpert.setArchive(false);
        }
        typeExpert.setArchive(true);
        typeExpert.setDateArchivage(new Date());
        typeExpertDao.save(typeExpert);
        return typeExpert;

    }

    @Override
    public TypeExpert desarchiver(TypeExpert typeExpert) {
    if (typeExpert.getArchive() == null) {
    typeExpert.setArchive(false);
    }
    typeExpert.setArchive(false);
    typeExpert.setDateArchivage(null);
    typeExpertDao.save(typeExpert);
    return typeExpert;
    }
