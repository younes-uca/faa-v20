...
import com.ird.faa.dao.EnseignementEtFormationDao;
...
@Service
public class EnseignementEtFormationChercheurServiceImpl extends AbstractServiceImpl<EnseignementEtFormation> implements EnseignementEtFormationChercheurService {
...
if(enseignementEtFormation!=null && enseignementEtFormation.getId() != null) {
        List<Enseignement> enseignements = enseignementService.findByEnseignementEtFormationId(enseignementEtFormation.getId());
        enseignementEtFormation.setEnseignements(enseignements);
        List<FormationContinue> formationContinues = formationContinueService.findByEnseignementEtFormationId(enseignementEtFormation.getId());
        enseignementEtFormation.setFormationContinues(formationContinues);
        List<ResponsabilitePedagogique> responsabilitePedagogiques = responsabilitePedagogiqueService.findByEnseignementEtFormationId(enseignementEtFormation.getId());
...
if(id != null ) {
    ...
    if(enseignementEtFormation !=null && enseignementEtFormation.getId() != null){
            List<List<Enseignement>> resultEnseignements= enseignementService.getToBeSavedAndToBeDeleted(enseignementService.findByEnseignementEtFormationId(enseignementEtFormation.getId()),enseignementEtFormation.getEnseignements());
            ...
            List<List<FormationContinue>> resultFormationContinues= formationContinueService.getToBeSavedAndToBeDeleted(formationContinueService.findByEnseignementEtFormationId(enseignementEtFormation.getId()),enseignementEtFormation.getFormationContinues());
            ...
            List<List<ResponsabilitePedagogique>> resultResponsabilitePedagogiques= responsabilitePedagogiqueService.getToBeSavedAndToBeDeleted(responsabilitePedagogiqueService.findByEnseignementEtFormationId(enseignementEtFormation.getId()),enseignementEtFormation.getResponsabilitePedagogiques());
...
if(foundedEnseignementEtFormation==null) return null;
else{
...
return list;
        ...
        private List<Enseignement> prepareEnseignements(EnseignementEtFormation enseignementEtFormation,List<Enseignement> enseignements){
        for(Enseignement enseignement:enseignements ){
        enseignement.setEnseignementEtFormation(enseignementEtFormation);
        }
        return enseignements;
        }
        private List<FormationContinue> prepareFormationContinues(EnseignementEtFormation enseignementEtFormation,List<FormationContinue> formationContinues){
        ...
        private List<ResponsabilitePedagogique> prepareResponsabilitePedagogiques(EnseignementEtFormation enseignementEtFormation,List<ResponsabilitePedagogique> responsabilitePedagogiques){
...
public int delete(EnseignementEtFormation enseignementEtFormation){
    if(enseignementEtFormation.getId()==null) return -1;
    EnseignementEtFormation foundedEnseignementEtFormation = findById(enseignementEtFormation.getId());
    if(foundedEnseignementEtFormation==null) return -1;
...
String query = "SELECT o FROM EnseignementEtFormation o where 1=1 ";
    query += SearchUtil.addConstraint("o", "chercheur.username", "=", currentUser.getUsername());
            ...
            query += SearchUtil.addConstraint( "o", "tempsEstimePourCetteAnnne","=",enseignementEtFormationVo.getTempsEstimePourCetteAnnne());
            query += SearchUtil.addConstraintMinMax("o","annee",enseignementEtFormationVo.getAnneeMin(),enseignementEtFormationVo.getAnneeMax());
            query += SearchUtil.addConstraintMinMax("o","tempsEstimePourCetteAnnne",enseignementEtFormationVo.getTempsEstimePourCetteAnnneMin(),enseignementEtFormationVo.getTempsEstimePourCetteAnnneMax());
    if(enseignementEtFormationVo.getEtatEtapeCampagneVo()!=null){
        query += SearchUtil.addConstraint( "o", "etatEtapeCampagne.id","=",enseignementEtFormationVo.getEtatEtapeCampagneVo().getId());
            query += SearchUtil.addConstraint( "o", "etatEtapeCampagne.code","LIKE",enseignementEtFormationVo.getEtatEtapeCampagneVo().getCode());
    }

    if(enseignementEtFormationVo.getChercheurVo()!=null){
        query += SearchUtil.addConstraint( "o", "chercheur.id","=",enseignementEtFormationVo.getChercheurVo().getId());
            query += SearchUtil.addConstraint( "o", "chercheur.numeroMatricule","LIKE",enseignementEtFormationVo.getChercheurVo().getNumeroMatricule());
    }

    if(enseignementEtFormationVo.getCampagneVo()!=null){
        query += SearchUtil.addConstraint( "o", "campagne.id","=",enseignementEtFormationVo.getCampagneVo().getId());
            query += SearchUtil.addConstraint( "o", "campagne.code","LIKE",enseignementEtFormationVo.getCampagneVo().getCode());
    }
...
return new ArrayList<>();
        ...
        private  void saveFormationContinues(EnseignementEtFormation enseignementEtFormation,List<FormationContinue> formationContinues){
        ...
        private  void saveResponsabilitePedagogiques(EnseignementEtFormation enseignementEtFormation,List<ResponsabilitePedagogique> responsabilitePedagogiques){
    ...
    private void findEtatEtapeCampagne(EnseignementEtFormation enseignementEtFormation){
        EtatEtapeCampagne loadedEtatEtapeCampagne =etatEtapeCampagneService.findByIdOrCode(enseignementEtFormation.getEtatEtapeCampagne());
...
private void associateEnseignement(EnseignementEtFormation enseignementEtFormation, List<Enseignement> enseignement) {
    if (ListUtil.isNotEmpty(enseignement)) {
        enseignement.forEach(e -> e.setEnseignementEtFormation(enseignementEtFormation));
    }
    }
private void associateFormationContinue(EnseignementEtFormation enseignementEtFormation, List<FormationContinue> formationContinue) {
    if (ListUtil.isNotEmpty(formationContinue)) {
        formationContinue.forEach(e -> e.setEnseignementEtFormation(enseignementEtFormation));
    }
    }
private void associateResponsabilitePedagogique(EnseignementEtFormation enseignementEtFormation, List<ResponsabilitePedagogique> responsabilitePedagogique) {
...
List<EnseignementEtFormation> resultat= enseignementEtFormationDao.findByChercheurUsernameAndCampagneId(username, compagneId);
    return resultat;
}
