...
@Component({
  ...
  styleUrls: ['./vie-institutionnelle-create-chercheur.component.css']
})
export class VieInstitutionnelleCreateChercheurComponent implements OnInit {
 ...
 public loadCampagne(username) {
    this.campagneService.findProgressCampagneByChercheurUsername(username).subscribe(data => {
     if (data != null && data.id != null) {
        let campagneVo = data;
        this.vieInstitutionnelleService.findByChercheurUsernameAndCampagneId(this.tokenService.getUsername(), campagneVo['id']).subscribe(vieInstitutionnelle => {
          this.isLoaded = false;
          if (vieInstitutionnelle) {
            this.msgsContents = 'Vous avez saisi les données vieInstitutionnelle de cette campagne';
            this.info = 'info'
            //this.selectedVieInstitutionnelle={ ...vieInstitutionnelle }['0']; TODO: in case non formulaire
            this.selectedVieInstitutionnelle.campagneVo=campagneVo;
            this.isLoaded = true;
          }
          else {
...
if(this.data) {
 this.selectedVieInstitutionnelle=this.data;
 }
