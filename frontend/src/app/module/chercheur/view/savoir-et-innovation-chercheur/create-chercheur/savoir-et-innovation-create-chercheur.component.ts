...
@Component({
  selector: 'app-savoir-et-innovation-create-chercheur',
  templateUrl: './savoir-et-innovation-create-chercheur.component.html',
  styleUrls: ['./savoir-et-innovation-create-chercheur.component.css']
})
export class SavoirEtInnovationCreateChercheurComponent implements OnInit {
 ...
 public loadCampagne(username) {
    this.campagneService.findProgressCampagneByChercheurUsername(username).subscribe(data => {
     if (data != null && data.id != null) {
        let campagneVo = data;
        this.savoirEtInnovationService.findByChercheurUsernameAndCampagneId(this.tokenService.getUsername(), campagneVo['id']).subscribe(savoirEtInnovation => {
          this.isLoaded = false;
          if (savoirEtInnovation) {
            this.msgsContents = 'Vous avez saisi les données savoirEtInnovation de cette campagne';
            this.info = 'info'
            //this.selectedSavoirEtInnovation={ ...savoirEtInnovation }['0']; TODO: in case non formulaire
            this.selectedSavoirEtInnovation.campagneVo=campagneVo;
            this.isLoaded = true;
          }
          else {
...
if(this.data) {
 this.selectedSavoirEtInnovation=this.data;
 }
