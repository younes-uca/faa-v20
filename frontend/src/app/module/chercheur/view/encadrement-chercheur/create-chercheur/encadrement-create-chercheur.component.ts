...
@Component({
  ...
  styleUrls: ['./encadrement-create-chercheur.component.css']
})
export class EncadrementCreateChercheurComponent implements OnInit {
 ...
 public loadCampagne(username) {
    this.campagneService.findProgressCampagneByChercheurUsername(username).subscribe(data => {
     if (data != null && data.id != null) {
        let campagneVo = data;
        this.encadrementService.findByChercheurUsernameAndCampagneId(this.tokenService.getUsername(), campagneVo['id']).subscribe(encadrement => {
          this.isLoaded = false;
          if (encadrement) {
            this.msgsContents = 'Vous avez saisi les données encadrement de cette campagne';
            this.info = 'info'
            //this.selectedEncadrement={ ...encadrement }['0']; TODO: in case non formulaire
            this.selectedEncadrement.campagneVo=campagneVo;
            this.isLoaded = true;
          }
          else {
...
if(this.data) {
 this.selectedEncadrement=this.data;
 }
