...
@Component({
  selector: 'app-enseignement-et-formation-create-chercheur',
  templateUrl: './enseignement-et-formation-create-chercheur.component.html',
  styleUrls: ['./enseignement-et-formation-create-chercheur.component.css']
})
export class EnseignementEtFormationCreateChercheurComponent implements OnInit {
 ...
 public loadCampagne(username) {
    this.campagneService.findProgressCampagneByChercheurUsername(username).subscribe(data => {
     if (data != null && data.id != null) {
        let campagneVo = data;
        this.enseignementEtFormationService.findByChercheurUsernameAndCampagneId(this.tokenService.getUsername(), campagneVo['id']).subscribe(enseignementEtFormation => {
          this.isLoaded = false;
          if (enseignementEtFormation) {
            this.msgsContents = 'Vous avez saisi les données enseignementEtFormation de cette campagne';
            this.info = 'info'
            //this.selectedEnseignementEtFormation={ ...enseignementEtFormation }['0']; TODO: in case non formulaire
            this.selectedEnseignementEtFormation.campagneVo=campagneVo;
            this.isLoaded = true;
          }
          else {
...
if(this.data) {
 this.selectedEnseignementEtFormation=this.data;
 }
